...
void main() async {
  ...
  for (var i = 0; i < entryCount; i++) {
    BloomData.setQueryData<Map<String, dynamic>>(
      ['user', i],
      {'id': i, 'name': 'User $i', 'email': 'user$i@bloom.dev', 'active': true},
  ...
  for (var i = 0; i < concurrentCallers; i++) {
    ...
    futures.add(BloomData.fetchQuery<String>(
      ['resource', keyId],
      () => mockApiFetch(keyId),
    ));
  }
  ...
  for (var i = 0; i < mutationCycles; i++) {
    final key = ['item', i];
    BloomData.setQueryData<int>(key, 100);
  ...
  for (var i = 0; i < 1000; i++) {
    BloomData.setQueryData(['item', i], i);
  }
  ...
  for (var i = 0; i < 10000; i++) {
    BloomData.setQueryData(['large_user', i], {
      'id': i,
