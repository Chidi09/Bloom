...
class DashboardPage extends BloomPage {
  ...
  return (
    <div className="bg-white/90 dark:bg-black backdrop-blur-2xl rounded-3xl overflow-hidden border border-slate-200/60 dark:border-zinc-800 shadow-2xl relative">
      {/* Top Bar */}
      <div className="px-5 py-3.5 bg-slate-100/70 dark:bg-zinc-950 border-b border-slate-200/60 dark:border-zinc-800 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="h-4 w-px bg-slate-300 dark:bg-zinc-800 mx-1.5" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 min-h-[480px]">
        ...
        <div className="lg:col-span-6 bg-[#FAFAFA]/60 dark:bg-black p-6 font-mono text-[12px] leading-relaxed overflow-x-auto relative select-none border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-zinc-800">
          <pre
            className="text-slate-900 dark:text-slate-200 leading-relaxed font-mono"
            dangerouslySetInnerHTML={{ __html: highlightDart(dartCode) }}
          />
        </div>

        {/* Right: Live Interactive Canvas Workbench */}
        <div className="lg:col-span-6 bg-slate-100/40 dark:bg-black/60 p-6 flex flex-col justify-between space-y-6">
          {/* Header Controls */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400">Zero rebuild boilerplate</span>
              </div>
            </div>

            {/* Segmented Screen Tabs */}
            <div className="flex p-0.5 bg-slate-200 dark:bg-zinc-900 rounded-xl text-[11px] font-mono font-bold border border-slate-300 dark:border-zinc-800">
              <button
                onClick={() => setActiveTab('revenue')}
                className={`px-3 py-1 rounded-lg transition-all ${
                  activeTab === 'revenue' ? 'bg-purple-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
                Revenue
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg transition-all ${
                  activeTab === 'activity' ? 'bg-purple-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
                Activity
              </button>
          ...
          {activeTab === 'revenue' ? (
            <div className="space-y-4">
              <div className="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 p-5 rounded-2xl shadow-xl relative overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs font-mono">
                <div className="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 p-3.5 rounded-xl">
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold block mb-0.5">Active Subscribers</span>
                  <span className="text-lg font-black text-slate-900 dark:text-white">{salesCount}</span>
                </div>
                <div className="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 p-3.5 rounded-xl">
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold block mb-0.5">Frame Latency</span>
            ...
            <div className="space-y-2.5">
              ...
              {activities.map((act) => (
                <div
                  key={act.id}
                  className="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 p-3 rounded-xl flex items-center justify-between text-xs animate-in fade-in"
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">{act.time}</p>
                    </div>
                  </div>
