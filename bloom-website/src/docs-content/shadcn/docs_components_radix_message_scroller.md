...
> Source: https://ui.shadcn.com/docs/components/radix/message-scroller
...
```
Copyimport { Message } from "@/components/ui/message"
import {
  MessageScroller,
...
```
Copy<div className="flex h-screen flex-col">
  <MessageScrollerProvider>
...
```
Copy// This tells the scroller to anchor the user's message for the next turn.
<MessageScrollerItem
  messageId={message.id}
  scrollAnchor={message.role === "user"}
/>```
...
```
Copy<MessageScrollerItem messageId="marcus-joined" scrollAnchor>
...
```
Copy// Keep 64px of the previous turn visible above the newly anchored row.
...
```
Copy<MessageScrollerProvider autoScroll>
...
```
Copy<MessageScrollerProvider defaultScrollPosition="last-anchor">
...
```
Copyconst MotionMessageScrollerItem = motion.create(MessageScrollerItem)```
...
```
Copyimport { useMessageScroller } from "@/components/ui/message-scroller"```

```
Copyconst { scrollToMessage, scrollToEnd, scrollToStart } = useMessageScroller()```
...
```
Copyimport { useMessageScrollerVisibility } from "@/components/ui/message-scroller"```

```
Copyconst { currentAnchorId, visibleMessageIds } = useMessageScrollerVisibility()```
...
```
Copyimport { useMessageScrollerScrollable } from "@/components/ui/message-scroller"```

```
Copyconst { start, end } = useMessageScrollerScrollable()```
...
```
Copy<MessageScrollerContent aria-busy={status === "streaming"}>
  {/* messages */}
</MessageScrollerContent>```
