...
# Dynamic Search
Implement server-side search for large registries.
...
```
CopyGET https://acme.com/r/registry.json?q=button&limit=50&offset=0```
...
```
Copyexport async function GET(request: NextRequest) {
...
```
Copycurl "https://acme.com/r/registry.json?q=button&limit=10"```
