...
#[test]
fn test_secrets_list_pagination_envelope_and_slicing() {
    use bytes::Bytes;
    ...
    let req = Request::new(
        Method::GET,
        Uri::from_static("/api/v1/secrets"),
        HeaderMap::new(),
        Bytes::new(),
    );
    ...
    let dummy_page1: Vec<serde_json::Value> = (0..100)
        .map(|i| serde_json::json!({ "id": format!("sec-{i}"), "key": format!("KEY_{i}") }))
        .collect();

    let env1 = pagination.envelope(&req, total, dummy_page1.clone());
    assert_eq!(env1["count"], 105);
    ...
    let req_p2 = Request::new(
        Method::GET,
        Uri::from_static("/api/v1/secrets?page=2"),
        HeaderMap::new(),
        Bytes::new(),
    );
    ...
    let dummy_page2: Vec<serde_json::Value> = (100..105)
        .map(|i| serde_json::json!({ "id": format!("sec-{i}"), "key": format!("KEY_{i}") }))
        .collect();
    ...
    assert_eq!(env2["results"].as_array().unwrap().len(), 5);
    ...
    let req_oversized = Request::new(
        Method::GET,
        Uri::from_static("/api/v1/secrets?page_size=500"),
        HeaderMap::new(),
        Bytes::new(),
    );
