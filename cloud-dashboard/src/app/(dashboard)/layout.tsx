...
interface SidebarNavProps {
  ...
  buildBadge: { count: number; color: "red" | "yellow" } | null;
  credentialBadge: { count: number; color: "red" | "yellow" } | null;
  appCount: number | null;
  projectCount: number | null;
  orgs: Array<{ id: string; name: string; slug: string; plan?: string }>;
  activeOrg:
...
export default function DashboardLayout({
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#000000] font-sans text-zinc-100 antialiased">
      {/* Vercel-style Left Navigation Sidebar (desktop) */}
      <aside className="border-border/60 hidden w-60 shrink-0 flex-col border-r bg-[#09090b] md:flex">
      ...
      <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
        <SheetContent
          side="left"
          className="flex w-72 flex-col gap-0 border-zinc-800 bg-[#09090b] p-0"
        >
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        {/* Top Header Bar */}
        <header className="flex h-12 shrink-0 items-center justify-between border-b border-zinc-800/60 bg-[#09090b]/80 px-3 backdrop-blur-md sm:px-6">
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              type="button"
              aria-label="Open navigation"
              onClick={() => setMobileNavOpen(true)}
              className="text-muted-foreground hover:text-foreground -ml-1 flex size-7 cursor-pointer items-center justify-center rounded-md transition-colors md:hidden"
            >
              <List className="size-4" />
            </button>
            <button
              type="button"
              className="hidden cursor-pointer items-center gap-1.5 text-xs font-medium text-zinc-300 transition-colors hover:text-zinc-100 sm:flex"
            >
        ...
        <main className="flex-1 overflow-y-auto bg-[#000000] p-3 sm:p-4 md:p-6">
          {children}
        </main>
      </div>
    </div>
