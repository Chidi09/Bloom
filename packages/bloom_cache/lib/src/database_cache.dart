...
class DatabaseCache extends BloomCache {
  ...
  Future<void> set<T>(String key, T value,
    ...
    await db.transaction((tx) async {
      await tx.execute(sql, [key, rawValue, expiresAt]);
      ...
      await tx.execute(
        'DELETE FROM $tagTableName WHERE key = ${db.dialect.placeholder(1)}',
        [key],
      );
  ...
  Future<void> delete(String key) async {
    ...
    await db.transaction((tx) async {
      await tx.execute(sql, [key]);
      await tx.execute('DELETE FROM $tagTableName WHERE key = $p1', [key]);
    });
  }
  ...
  Future<void> clear() async {
    ...
    await db.transaction((tx) async {
      ...
      await tx.execute('DELETE FROM $tagTableName');
    });
  }
  ...
  /// Removes every entry labelled with ANY of the given [tags] inside an atomic transaction.
  ///
  ...
  Future<void> invalidateTags(List<String> tags) async {
    ...
    await db.transaction((tx) async {
      ...
      final p1 = db.dialect.placeholder(1);
      for (final tag in tags.toSet()) {
        final rows = await tx.fetchAll(
          'SELECT key FROM $tagTableName WHERE tag = $p1',
          [tag],
        );
      ...
      for (final key in keys) {
        await tx.execute('DELETE FROM $tableName WHERE key = $p1', [key]);
        await tx.execute('DELETE FROM $tagTableName WHERE key = $p1', [key]);
      }
    });
  }

  /// Deletes all expired entries from the cache table and their companion tag rows
  /// inside an atomic database transaction.
  ///
  ...
  Future<int> pruneExpired() async {
    ...
    return await db.transaction((tx) async {
      final rows = await tx.fetchAll(
        'SELECT key FROM $tableName WHERE expires_at IS NOT NULL AND expires_at < $p1',
        [now],
      );
      for (final row in rows) {
        final key = row.tryStringByName('key') ?? row.tryString(0);
      ...
      final sql =
          'DELETE FROM $tableName WHERE expires_at IS NOT NULL AND expires_at < $p1';
      return await tx.execute(sql, [now]);
    });
  }
}
