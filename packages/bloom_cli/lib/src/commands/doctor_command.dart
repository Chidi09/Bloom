...
import '../utils/manifest_validator.dart';
...
class DoctorCommand extends Command<int> {
  ...
  final String description =
      'Validates environment, toolchain, project configuration, and CI health.';
  ...
  DoctorCommand() {
    ...
    argParser.addFlag(
      'ci',
      help:
          'Runs strict CI continuous diagnostic verification and fails on any risk.',
      defaultsTo: false,
    );
    argParser.addOption(
      'project-dir',
  ...
  Future<int> run() async {
    ...
    if (javaRes.exitCode == 0) {
      final sdkStatus =
          androidHome != null ? 'SDK: $androidHome' : 'Java present';
      print('${Ansi.green}✔ OK${Ansi.reset} ${Ansi.dimText('($sdkStatus)')}');
    } else {
      print(
          '${Ansi.yellow}⚠ JDK not found (Required for Android builds)${Ansi.reset}');
    }
    ...
    if (Platform.isMacOS) {
      ...
      if (xcodeRes.exitCode == 0) {
        ...
        print(
            '${Ansi.yellow}⚠ Xcode command line tools not found${Ansi.reset}');
      }
    }
    ...
    try {
      ...
      if (shorebirdRes.exitCode == 0) {
        final versionLine =
            shorebirdRes.stdout.toString().trim().split('\n').first;
        print(
            '${Ansi.green}✔ OK${Ansi.reset} ${Ansi.dimText('($versionLine)')}');
      } else {
        print(
            '${Ansi.dimText('ℹ Optional (run curl -sSf https://raw.githubusercontent.com/shorebirdtech/install/main/install.sh | bash)')}');
      }
    } catch (_) {
      print(
          '${Ansi.dimText('ℹ Optional (run curl -sSf https://raw.githubusercontent.com/shorebirdtech/install/main/install.sh | bash)')}');
    ...
    if (localIpResult.isLoopback) {
      ...
      print(
          '${Ansi.green}✔ OK${Ansi.reset} ${Ansi.dimText('(LAN IP: ${localIpResult.ip})')}');
    }
    ...
    print(
        '${Ansi.green}✔ OK${Ansi.reset} ${Ansi.dimText('(${Platform.operatingSystem}, ${Platform.operatingSystemVersion})')}');

    stdout.write('  Checking CPU Cores... ');
    print(
        '${Ansi.green}✔ OK${Ansi.reset} ${Ansi.dimText('(${Platform.numberOfProcessors} cores)')}');

    stdout.write('  Checking Process Memory... ');
    final rssMb = (ProcessInfo.currentRss / (1024 * 1024)).toStringAsFixed(1);
    print(
        '${Ansi.green}✔ OK${Ansi.reset} ${Ansi.dimText('(current CLI process RSS: ${rssMb} MB)')}');

    stdout.write('  Checking Dart Executable... ');
    print(
        '${Ansi.green}✔ OK${Ansi.reset} ${Ansi.dimText('(${Platform.resolvedExecutable})')}');
    ...
    if (project != null) {
      ...
      if (project.bloomYamlFile.existsSync()) {
        final issues =
            BloomManifestValidator.validateFile(project.bloomYamlFile);
        if (issues.isEmpty) {
          final config = project.loadBloomConfig();
          print(
              '  ${Ansi.green}✔ bloom.yaml valid${Ansi.reset} ${Ansi.dimText('(schema: ${config['schema'] ?? 1})')}');
        } else {
          print('  ${Ansi.red}✖ bloom.yaml invalid${Ansi.reset}');
      ...
      if (envFile.existsSync()) {
        ...
        print(
            '  ${Ansi.yellow}⚠ .env file missing (create a .env file in project root)${Ansi.reset}');
      }

      final routes = project.scanRoutes();
      print(
          '  ${Ansi.green}✔ ${routes.length} filesystem route(s) discovered${Ansi.reset}');
      for (final r in routes) {
        print(
            '    ${Ansi.dimText('• ${r.routePath} -> ${r.componentClassName} (${r.relativeFilePath})')}');
      }
    } else {
    ...
    if (allGood) {
      ...
      print(
          Ansi.warn('Some checks did not pass. Please resolve above items.\n'));
      return 1;
    }
  }
  ...
  Future<void> _checkBloomCliVersion() async {
            ...
            if (latest is Map<String, dynamic>) {
              ...
              if (latestVersion != null && latestVersion.isNotEmpty) {
                if (latestVersion == _bloomCliVersion) {
                  print(
                      '${Ansi.green}✔ Up to date${Ansi.reset} ${Ansi.dimText('(v$_bloomCliVersion)')}');
                } else {
                  print(
                      '${Ansi.yellow}⚠ Update available${Ansi.reset} ${Ansi.dimText('(installed v$_bloomCliVersion, latest v$latestVersion)')}');
                }
                return;
              }
            }
    ...
    print(Ansi.dimText(
        'ℹ Unable to check for updates (offline or not published) (installed v$_bloomCliVersion)'));
  }
  ...
  Future<int> _runCiDoctor(BloomProject? project, Directory projectDir) async {
    ...
    if (project == null) {
      print(Ansi.error(
          '✖ CI Check Failed: Not inside a valid Bloom project (${projectDir.path})\n'));
      return 1;
    }
    ...
    if (!project.bloomYamlFile.existsSync()) {
      print(Ansi.error(
          '✖ [CONFIG] Missing required bloom.yaml configuration file.'));
      ciFailed = true;
    } else {
      final issues = BloomManifestValidator.validateFile(project.bloomYamlFile);
      if (issues.isEmpty) {
        print(Ansi.success('✔ [CONFIG] Valid bloom.yaml configuration'));
      } else {
        print(Ansi.error('✖ [CONFIG] Invalid bloom.yaml configuration:'));
        for (final issue in issues) {
          print(Ansi.error('  • $issue'));
        }
        ciFailed = true;
      }
    }
    ...
    if (auditReport.hasVulnerabilities) {
      print(Ansi.error(
          '✖ [SECURITY] Discovered ${auditReport.findings.length} vulnerable dependency advisory/advisories!'));
      ciFailed = true;
    } else {
    ...
    if (secretResult.hasSecrets) {
      print(Ansi.error(
          '✖ [SECRETS] Exposed secret tokens or private keys detected in codebase!'));
      ciFailed = true;
    } else {
    ...
    try {
      ...
      if (modules.isEmpty) {
        print(Ansi.success(
            '✔ [AUTOLINK] Native module autolink verified with no version conflicts'));
      } else {
    ...
    final deploymentChecker = const DeploymentDoctorChecker();
    final deployReport =
        await deploymentChecker.check(project: project, strictCi: true);
    if (!deployReport.isPassed) {
      for (final item
          in deployReport.items.where((i) => !i.isHealthy && !i.isWarning)) {
      ...
      print(Ansi.success(
          '✔ [DEPLOY] Deployment configuration & container checks passed'));
    }

    print('');
    if (ciFailed) {
      print(Ansi.error(
          '✖ Continuous CI Verification Failed! Please address above diagnostic errors.\n'));
      return 1;
    }
