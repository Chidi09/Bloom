...
class BloomModuleCodeGenerator {
  ...
  static ModuleParsedDefinition? parseModuleDefinition(String content, File sourceFile) {
    // 1. Extract @BloomModule metadata
    final moduleMatch = RegExp(
      r"@BloomModule\s*\(\s*name:\s*'([^']+)'(?:,\s*version:\s*'([^']+)')?(?:,\s*description:\s*'([^']+)')?",
    ).firstMatch(content);
    ...
    final constantMatches = RegExp(
      r'@BloomConstant\(\)\s*([A-Za-z0-9_<>?]+)\s+get\s+([A-Za-z0-9_]+);',
    ).allMatches(content);
    for (final m in constantMatches) {
      constants.add(ModuleConstantSpec(type: m.group(1)!, name: m.group(2)!));
    }

    // 4. Extract Async Functions
    final methods = <ModuleMethodSpec>[];
    final asyncFuncMatches = RegExp(
      r'@BloomAsyncFunction\((?:thread:\s*NativeThread\.([A-Za-z0-9_]+))?\)\s*Future<([\s\S]+?)>\s+([A-Za-z0-9_]+)\s*\(([\s\S]*?)\);',
    ).allMatches(content);
    ...
    for (final m in eventMatches) {
      streams.add(ModuleStreamSpec(type: m.group(1)!.trim(), name: m.group(2)!, isEvent: true));
    }
    ...
    for (final m in streamMatches) {
      streams.add(ModuleStreamSpec(type: m.group(1)!.trim(), name: m.group(2)!, isEvent: false));
    }

    // 7. Extract Views
    final views = <ModuleViewSpec>[];
    final viewMatches = RegExp(
      r"@BloomView\(\s*name:\s*'([^']+)'\)\s*(?:void|Widget)\s+([A-Za-z0-9_]+)\s*\(([\s\S]*?)\);",
    ).allMatches(content);
    for (final m in viewMatches) {
      final viewName = m.group(1)!;
      final rawParams = m.group(3)?.trim() ?? '';
      views.add(ModuleViewSpec(name: viewName, props: _parseParams(rawParams)));
    }
  ...
  static List<ModuleMethodParam> _parseParams(String rawParams) {
    ...
    for (var p in parts) {
      ...
      String? defaultValue;
      if (p.contains('=')) {
        final defSplit = p.split('=');
        p = defSplit[0].trim();
        defaultValue = defSplit[1].trim();
      }

      final typeAndName = p.split(RegExp(r'\s+'));
      if (typeAndName.length >= 2) {
        final type = typeAndName[0].trim();
        final name = typeAndName[1].trim();
  ...
  static String generateDartBridge(ModuleParsedDefinition def, String baseFileName) {
    ...
    for (final c in def.constants) {
      ...
      buffer.writeln('  ${c.type} get ${c.name} => getProperty<${c.type}>(\'${c.name}\') ?? \'\';');
      buffer.writeln();
    }
      ...
      final paramsSig = v.props.isEmpty
          ...
          : '{' +
              v.props
                  ...
                  .join(', ') +
              '}';
      buffer.writeln('  void ${v.name}($paramsSig) {}');
      buffer.writeln();
    }
  ...
  static String generateSwiftBridge(ModuleParsedDefinition def) {
    ...
    for (final m in def.methods) {
      ...
      buffer.writeln('                    let res = try await bridge?.${m.name}($swiftArgs)');
  ...
  static String generateKotlinBridge(ModuleParsedDefinition def, String ktDirPath) {
    ...
    buffer.writeln('// AUTO-GENERATED BY BLOOM MODULE GENERATOR. DO NOT EDIT.');
    buffer.writeln('package dev.bloom.modules.$name');
    buffer.writeln();
