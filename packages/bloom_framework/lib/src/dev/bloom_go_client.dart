...
class BloomGoClient {
  ...
  /// Fetch application manifest from the active dev server.
  Future<BloomProjectManifest> fetchManifest(String httpBaseUrl) async {
    logger.info('BloomGoClient: Fetching project manifest from $httpBaseUrl/manifest.json...');
    final response = await _httpClient.get(Uri.parse('$httpBaseUrl/manifest.json'));
  ...
  /// Register device pairing with the dev server.
